...
- 실 MySQL clean 로드(V1~V82) + `ddl-auto=validate` 통합 테스트.
...
1. **프로필 태그 교체** — V81 마이그레이션 + `SkinProfileFields.tsx`·`api/auth.ts`(`Concern` 유니온)
   + `SkinProfileFields.css` + DESIGN.md §8 세 절 추가. 화면 태스크이므로 스크린샷 DoD.
2. **규칙 배포 API** — V82 테이블·시드 + `routine` 패키지 컨트롤러·서비스·DTO + `SecurityConfig` 1줄.

Flyway 번호(V81 / V82)를 계획서에서 미리 고정하므로 두 터미널이 마이그레이션 파일을 두고
