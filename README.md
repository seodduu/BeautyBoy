...
<table>
  ...
  <tr>
    ...
    <td><h3>"돈과 재고는 서버, 취향은 클라이언트" — 행동 점수는 기기(localStorage)에서 계산, 규칙의 진실만 서버가 배포. 서버 참조 구현을 만들어 k6+Grafana로 A/B 실측: 개인화 왕복의 소멸(기기 계산 0.024ms) · 행동 이력이 기기를 떠나지 않음(화면당 4.7KiB 미전송) · 캐시로 접을 수 없는 확장 축을 애초에 제거</h3></td>
  </tr>
  <tr>
    <td><h3>성분 정보</h3></td>
    <td>전성분 텍스트 나열</td>
