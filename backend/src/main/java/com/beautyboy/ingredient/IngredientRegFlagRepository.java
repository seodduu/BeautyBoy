...
public interface IngredientRegFlagRepository extends JpaRepository<IngredientRegFlag, Long> {
     ...
     * 반환 행: [ingredient_id, name, inci_name, summary, flag_type(NULL 가능), source_ref(NULL 가능), sort_order].
     */
    @Query(value = "SELECT gi.ingredient_id, i.name, i.inci_name, i.summary, f.flag_type, f.source_ref, gi.sort_order "
