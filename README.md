...
<p align="center">
  ...
  뭘 사야 할지 몰라도 된다. <strong>루틴을 따라가면, 구매가 끝나 있다.</strong><br/>
  피부 프로필 기반 개인화, 성분 궁합 진단, 토스 결제까지
  전 구간이 실API로 동작한다.
</p>
...
<p align="center">
  ...
  &nbsp;·&nbsp;
  <a href="https://medium.com/@SeoDDuu/beautyboy-%ED%8A%B8%EB%9F%AC%EB%B8%94%EC%8A%88%ED%8C%85-d5637a4bc448">트러블슈팅 (Medium)</a>
</p>
...
<table>
  ...
  <tr>
    <td><h3>주문 취소 · 환불</h3></td>
    <td><h3>수량 단위 부분 취소 — 환불액은 주문 시점 스냅샷 단가로 서버가 확정하고, 재고 복원과 랭킹 판매 집계 차감까지 함께 되돌림</h3></td>
  </tr>
  <tr>
    <td><h3>리뷰 · 문의 · 마이페이지 · Admin</h3></td>
    <td><h3>리뷰 작성·수정·삭제(피부타입 스냅샷 저장), 상품 문의, 프로필·배송지 관리, 관리자 CRUD까지 전 구간 실API 동작</h3></td>
  </tr>
</table>
...
<h2>↩️ 07. 주문 취소 — 수량 단위 부분 취소</h2>

<h3>취소할 항목과 수량을 골라 부분 취소합니다. 환불액은 주문 시점 스냅샷 단가로 서버가 확정하고, 재고와 랭킹 판매 집계도 함께 되돌립니다.</h3>

<img src="docs/demo/07-order-cancel.gif" width="100%" alt="주문 취소 — 수량 단위 부분 취소" />

<h2>👤 08. 마이페이지 — 프로필 변경</h2>

<h3>피부 프로필을 바꾸면 메인·루틴 추천이 즉시 따라옵니다.</h3>

<img src="docs/demo/08-profile-edit.gif" width="100%" alt="마이페이지 — 프로필 변경" />
...
<table>
  ...
  <tr>
    ...
    <td><h3>"세럼이 뭔지" 몰라도 3단계에 도착하면 지금 사야 할 것이 나온다</h3></td>
  </tr>
  <tr>
    <td><h3>개인화 방식</h3></td>
    <td>서버가 행동 로그를 수집해 추천 배치를 돌림</td>
    <td><h3>행동 점수는 기기(localStorage)에서 계산, 규칙의 진실만 서버가 배포. 서버 참조 구현을 만들어 k6+Grafana로 A/B 실측: 개인화 왕복의 소멸(기기 계산 0.024ms) · 행동 이력이 기기를 떠나지 않음(화면당 4.7KiB 미전송) · 캐시로 접을 수 없는 확장 축을 애초에 제거</h3></td>
  </tr>
  <tr>
    <td><h3>성분 정보</h3></td>
    <td>전성분 텍스트 나열</td>
