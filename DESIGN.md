...
**세트 진입 CTA** — `/main` 히어로 밴드 안, lede 아래 — `{button-primary-on-dark}`
(흰 채움 + 검정 글자 + `{rounded.full}`)에 화살표 `→`를 붙인다.
- **텍스트 링크로 두지 않는다.** 이 자리는 세트 페이지로 보내는 주 전환 경로이므로 액션답게
  세운다. 밑줄 텍스트로 두면 lede 끝에 매달린 각주처럼 읽히고, 크기를 키워 세우면
  "세 번째 헤드라인"이 되어 제목과 위계가 겹친다.
- **크기로 소리치지 않고 채움으로 무게를 준다.** 글자는 `{typography.button}`(14px/600) 그대로 —
  히어로의 위계는 제목(36px/400) → lede(18px/400) → CTA(면으로 구분)로 정리된다.
- 검정 밴드 위의 흰 채움이라 같은 화면 픽 카드의 `바로 담기`(검정 알약)와 색이 반전돼
  서로 경쟁하지 않는다. "화면당 검정 알약은 하나" 규칙과 충돌하지 않는다.
- **lede 마지막 문장이 이 CTA를 받아야 한다.** "아래로 내리세요"로 끝낸 뒤 다른 화면으로
  보내는 버튼을 놓으면 문장과 액션이 충돌한다. 두 경로(단계별·세트)를 한 문장에 담는다.
...
**`route-error`** — 라우트 오류·없는 페이지 화면
- **두 사건이 한 화면을 쓴다**: 라우트 렌더 중 잡히지 않은 예외, 그리고 매칭되는 경로가 없는
  주소(404). 손님에게는 "여기서 더 갈 수 없다"는 같은 사건이고, 화면을 둘로 나누면 문구만 다른
  코드가 두 벌 생긴다.
- **헤더·푸터 안에서 렌더한다.** 오류가 났다고 탐색 수단까지 뺏지 않는다 — 손님은 이 화면에서
  곧장 다른 카테고리로 갈 수 있어야 한다. 셸을 통째로 대체하는 전면 화면이 아니다.
- 골격은 `empty-state`와 같다: 제목 `{typography.heading-sm}` `{colors.ink}` + 설명
  `{typography.body}` `{colors.graphite}` + 행동 버튼 2개. 세로 여백은 위아래 `{spacing.section}`,
  가운데 정렬, 최대폭은 리딩 밴드 규칙(한글 25–40자)에 맞춰 가둔다.
- 행동: `{button-primary}` "다시 시도" · `{button-ghost}` "홈으로". "에러는 복구 경로를 함께
  준다"(UX 계약 — 피드백 절)의 적용 지점이다. 버튼 하나만 두지 않는다.
- **배경 채움·`{colors.signal-danger}` 테두리 금지.** 오류 화면은 경고 배너가 아니라 빈 상태의
  사촌이다 — 무채색으로 둔다(액센트를 배경으로 칠하지 않는다는 전역 규칙 그대로).
- 문구는 두 갈래뿐이다. 404는 "요청하신 페이지를 찾을 수 없어요", 그 외는 "화면을 불러오지
  못했어요". **원인 문자열(예외 메시지·스택)은 화면에 내지 않는다** — 손님이 그것으로 할 수
  있는 일이 없고, 내부 구조를 흘린다.
- 접근성: 컨테이너에 `role="alert"`, 제목은 `<h1>`(이 화면에서는 오류가 문서의 주제다).
