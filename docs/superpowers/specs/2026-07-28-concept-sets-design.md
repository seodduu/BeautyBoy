...
작성일: 2026-07-28. 상태: **부분 대체됨** — 후속 설계 `2026-07-28-sets-page-design.md` 참고.

- 여전히 유효: §4 컨셉 파생 사다리, §5 `useComposer(concernOverride)`·`composeStep` 정규화,
  §7 에러·폴백 정책, §8 범위 제외, §9의 `setConcepts.test.ts`·`composer.test.ts` 케이스.
- 대체됨: §1~§3(배치·탭 UX), §6(`SetTabs` 컴포넌트) — `/main` 탭은 철거됐고 세트 비교는
  전용 페이지 `/sets`가 맡는다. §9의 `Main.test.tsx` 절은 `/main` 회귀 단언으로 교체됐다.
...
| 프로필 미입력 회원 | **고정 대표 3종 폴백**(모공·트러블·보습) + 프로필 등록 유도 문구 |
...
- 폴백 사용자에게는 탭 아래에 안내 한 줄 + 프로필 입력 링크(`/mypage` 프로필).
  `/main`은 `RequireAuth` 뒤라 비로그인 사용자는 애초에 도달할 수 없다 — 폴백의 실제
  도달 경로는 **"프로필 미입력 회원" + `['me']` 조회 실패**뿐이다.
...
/**
 * 최후 폴백 3종 — 프로필 미입력 회원에게 보이는 대표 컨셉.
...
- **알려진 트레이드오프**: 단독 대체로 `signals.concerns`가 항상 1개가 되므로,
  `pickConcernReason`이 순회할 고민도 1개뿐이다. 고민을 3개 고른 회원 기준으로는 원래
  3개 고민 각각의 타겟 규칙을 순서대로 시도할 수 있었지만(시드 `concern_target_rule`은
  고민 1종당 2행), 세트 탭에서는 그 컨셉 하나의 규칙만 시도하므로 reason 문장이 나오는
  단계 수가 줄어들 수 있다. 세트 차별화를 위한 의도된 대가이며, 후속 과제로 남긴다.
...
| 상황 | 동작 |
|---|---|
| 프로필 미입력 회원 (`/main`은 `RequireAuth` 뒤라 비로그인은 도달 불가) | 폴백 3종 탭 + "프로필을 등록하면 맞춤 세트로 바뀌어요" 안내. 조합기는 기존대로 인기순 기준선. |
| `['me']` 조회 실패 | 프로필 미입력과 동일 취급 (폴백 3종). 던지지 않는다. |
...
| 프로필 미입력 회원 렌더 | 폴백 3종 라벨 + 안내 문구가 보인다 |
...
- DoD:
  ...
  3. **개발서버를 띄워 스크린샷으로 확인**: (a) 탭 3개 렌더, (b) 탭 전환 시 세럼 단계
     픽이 바뀜, (c) 프로필 미입력 폴백 문구. 스크린샷 경로를 보고서에 남기고 리뷰어가 열어 본다.
